...
async function saveLayoutFile() {
    ...
    if (screenBgPicker) {
        currentConfig._bgColor = screenBgPicker.value;
        renderPreview();
    }
...
function handleCloseRequest() {
    saveCurrentFormInputs();
    const screenBgPicker = document.getElementById('screen-bg-picker');
    const currentBg = screenBgPicker ? screenBgPicker.value.toLowerCase() : (currentConfig._bgColor || '#0f1115').toLowerCase();
    const savedBg = (lastSavedConfig && lastSavedConfig._bgColor ? lastSavedConfig._bgColor : (currentConfig._bgColor || '#0f1115')).toLowerCase();
    const bgChanged = currentBg !== savedBg;
    const hasUnsaved = bgChanged || (lastSavedConfig && (JSON.stringify(currentConfig) !== JSON.stringify(lastSavedConfig)));
    if (hasUnsaved) {
